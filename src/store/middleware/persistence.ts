...
import {
  persistenceManager,
  type PersistedState,
} from "../../services/persistence/manager";
    ...
    try {
      ...
      if (isValidPersistedState(persistedState) && options.onRehydrateStorage) {
        ...
        const stateToSet = {
          ...
          cardValues: Array.isArray(config.cardValues)
            ? config.cardValues
            : [1, 2, 3, 5, 8, 13, "?", "coffee"],
          isConfigured: Boolean(config.isConfigured),
          currentStory:
            typeof game.currentStory === "string" ? game.currentStory : "",
          isRevealing: Boolean(game.isRevealing),
          timer: typeof game.timer === "number" ? game.timer : 60,
          timerState:
            game.timerState && typeof game.timerState === "object"
              ? {
                  ...defaultTimerState,
                  ...game.timerState,
                  settings: {
