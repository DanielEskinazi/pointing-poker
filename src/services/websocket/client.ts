...
export class WebSocketClient {
  ...
  private setupConnectionHandlers() {
    ...
    this.socket.on("connect_error", (error) => {
      const timestamp = new Date().toISOString();
      console.error(
        `[WebSocket][${timestamp}] Connection error:`,
        error.message
      );
  ...
  private setupEventHandlers() {
    ...
    this.socket.on(ServerEvents.PLAYER_PROMOTED, (data) => {
      ...
      if (data.previousHost) {
        useGameStore
          .getState()
          .handlePlayerUpdated({ player: data.previousHost });
      }
    });
  ...
  private setupActivityListeners() {
    ...
    (this as unknown as { activityHandler: () => void }).activityHandler =
      throttledHandler;
    (this as unknown as { activityEvents: string[] }).activityEvents =
      activityEvents;
  }
  ...
  updateStory(
    ...
    this.emit(ClientEvents.STORY_UPDATE, {
      storyId,
      title,
      description,
      finalEstimate,
  ...
  adjustTimer(adjustmentSeconds: number) {
    this.emit(ClientEvents.TIMER_ADJUST, { adjustmentSeconds });
  }
